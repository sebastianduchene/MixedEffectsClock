#!/usr/bin/env python3
"""Score the 50-taxon simulation against its known truth. Usage: check_sim50.py <stem> [burnin]"""
import math, sys

stem   = sys.argv[1] if len(sys.argv) > 1 else "sim50_ME_beast2"
...
TRUTH = {"clockRate": 0.005, "coefficient": math.log(2.0)}
...
if "ucldStdev" in col:
    ...
    pulled = med < PRIOR_MEAN_DISPERSION / 3.0
    near0  = lo < 0.01
    if not (pulled and near0): bad += 1
    print("%-14s %10.5g %11.5g  [%9.5g,%9.5g] %8.0f  %s"
          % ("ucldStdev", 0.0, med, lo, hi, e,
             "piled at 0" if (pulled and near0) else "NOT pulled to 0"))
    print("               truth 0 is on the boundary, so coverage is not the test;")
    print("               prior mean is %.4g and the posterior median is %.4g" % (PRIOR_MEAN_DISPERSION, med))
print()
for name in EXTRA:
...
print()
for name, want in [("nPainted.X1",4),("nPainted.background",94),("nPainted.multiple",0)]:
    if name in col:
